...
class DataProc():
    ...
    def get_all_bykey(self,key,pindex_list):
        """
        Queries pagedata table by key and list of pindex values.  Returns dictionary of pindex-->(dates,vals) for tsarray insertion
           queries should be filtered by max id, which is the latest entry for each query in case of duplicates
        """
...
class TsArray():
    ...
    def add_array(self,array,name):
        ...
        assert array.shape[0] == len(self.p2r.keys())
        assert array.shape[1] == len(self.dt_list)
    ...
    def merge_tsarray(self,tsa_in,source_array,new_name=None):
        #adds array from external tsarray object to existing array where dates and rows match
        # fits external data into new array
        new_dates = tsa_in.dt_list
        common_dates = list(set(self.dt_list) & set(new_dates))
        row_map=list(self.p2r[pindex] for pindex in list(tsa_in.r2p[i] for i in range(len(tsa_in.r2p.keys()))))
        arr_in = tsa_in.arrays[source_array]
        tmp_arr = self.new_array()
        for dt in common_dates:
            source_c = tsa_in.d2c[dt]
            dest_c = self.d2c[dt]
            tmp_arr[row_map,dest_c] = arr_in[:,source_c]
        if new_name is None:
            arr_name = source_array
        else:
            arr_name = new_name
        self.add_array(tmp_arr,arr_name)

    def splice_data(self,arr1,arr2,dt_list):
        """
        Data in arr1 is overwritten with data from arr2 for the given dates
        """
        data1 = self.arrays[arr1]
        data2 = self.arrays[arr2]
        for dt in dt_list:
            col = self.d2c[dt]
            data1[:,col] = data2[:,col]
    ...
    def summary(self):
        ...
        for frame in frame_list:
            ...
            if len(valid_dt) == 0:
                print "NO VALID DATA IN FRAME %s" % frame
                continue
    ...
    def truncate(self,start_dt,end_dt):
        """
        returns a copy of self restricted to date range (inclusive)
        """
        if start_dt not in self.dt_list or end_dt not in self.dt_list:
            print "ERROR, truncate range incorrect!"
        new_ndays = (end_dt - start_dt).days + 1
        new_dt_list = list(start_dt + datetime.timedelta(days=i) for i in range(new_ndays))
        series_list = list(self.p2r.keys())
        tsa = TsArray(series_list,new_dt_list)
        tsa.p2i = self.p2r
        tsa.i2p = self.r2p
        for aname,arr in self.arrays.iteritems():
            tmp_arr = tsa.new_array()
            for dt in new_dt_list:
                source_i = self.d2c[dt]
                dest_i = tsa.d2c[dt]
                tmp_arr[:,dest_i] = arr[:,source_i]
            tsa.add_array(tmp_arr,aname)
        return tsa
...
class AggScheme():
    ...
    def get_agg_scheme(self,scheme_name,pterm_lookup,csv_in):
        ...
        with open(csv_in,'r') as track_f:
            ...
            if "MASTER" in hd_str:
                rows = [(row[0].upper().strip(), row[1].upper().strip(), row[2].upper().strip(), list(int(row[i]) for i in range(3,len(row)) if len(row[i]) > 0)) for row in reader]
            else:
                print "Error in CSV format, check for correct column content!"
                sys.exit()
        track_lookup = {}
        for track_t in rows:
            track_name = track_t[0]
            track_master = track_t[1]
            search_modifiers = list(sm.strip().upper() for sm in track_t[2].split(',') if len(sm) > 0)
            #include group/track name as search key
            search_terms = list(set([track_name,] + search_modifiers))
            st_match = []
            st_notmatch = []
            for sterm in search_terms:
                is_neg = re.search("^NOT\s",sterm.upper())
                if is_neg:
                    nm_index = is_neg.span()
                    nm_term = sterm[nm_index[1]::]
                    #remove extra spaces
                    nm_clean = " ".join(st.lower() for st in nm_term.split())
                    st_notmatch.append(nm_clean)
                else:
                    m_clean = " ".join(st.lower() for st in sterm.split())
                    st_match.append(m_clean)
            tindex_include = list(tindex for tindex in track_t[3] if tindex > 0)
            tindex_exclude = list(-tindex for tindex in track_t[3] if tindex < 0)
            track_lookup[track_name] = {"include":tindex_include,"exclude":tindex_exclude,"master":track_master,
                                        "st_inc":st_match,"st_ex":st_notmatch}
        ...
        for gname,tdict in track_lookup.iteritems():
            ...
            if len(self.scheme["groups"]) == 0:
                gdict = {"group_name":group_name,"group_pages":final_pages,"group_master":group_master,
                         "st_inc":tdict["st_inc"],"st_ex":tdict["st_ex"]}
    ...
    def show(self,full=False,slug_lookup=None):
        ...
        for gn,gdict in enumerate(self.scheme["groups"]):
            ...
            st_inc = " | ".join(gdict["st_inc"])
            st_ex = " | ".join(gdict["st_ex"])
            n_inc = len(gdict["st_inc"])
            n_ex = len(gdict["st_ex"])
            n_pages = len(group_pages)
            if n_ex > 0:
                ex_out = "NOT: "+st_ex
            else:
                ex_out = ""
            print "    Group: %20s = %8s contains %5d pages, search matching: %s %s" % (group_name,group_master,n_pages,st_inc,ex_out)
            if full and slug_lookup is not None:
                print "     Included Search Terms:",st_inc
                if n_ex > 0:
                    print "     Excluded Search Terms:",st_ex
                for page in group_pages:
                    print "       --> Group %16s includes page %5g with title %s" % (group_name,page,slug_lookup.get(page,"Unknown"))
    ...
    def update_tsa(self,proc,target_tsa,target_array,local=False,file=None):
        ...
        if local and file is not None:
            ...
            if set(tsa_in.dt_list) == set(target_tsa.dt_list):
                ...
                print "Dates do not match, re-fetching all data!"
